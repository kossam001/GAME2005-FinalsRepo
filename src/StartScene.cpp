...
void StartScene::update()
{
	...
	CollisionManager::AABBCheck(m_pHero, m_pPlatform);
	if (m_pPlatform->getIsColliding() && m_gravityEnabled)
	{
		m_isGrounded = true;
	}
	else if (m_gravityEnabled) // if the gravity is disabled, we should be able to move left and right
	{
		m_isGrounded = false;
	}
...
void StartScene::m_move()
{
	...
	if(m_moveState == MOVE_UP && !m_gravityEnabled)
	{
		m_velocity = glm::vec2(m_velocity.x, -1.0f);
	}

	if (m_moveState == MOVE_DOWN && !m_gravityEnabled)
	{
		m_velocity = glm::vec2(m_velocity.x, 1.0f);
	}

	if ((m_moveState == MOVE_LEFT && m_isGrounded) || (m_moveState == MOVE_LEFT && !m_gravityEnabled))
	{
		m_velocity = glm::vec2(-1.0f, m_velocity.y);
	}

	if ((m_moveState == MOVE_RIGHT && m_isGrounded) || (m_moveState == MOVE_RIGHT && !m_gravityEnabled))
	{
		m_velocity = glm::vec2(1.0f, m_velocity.y);
	}
	...
	{
		m_velocity = glm::vec2(0, m_gravity);
	}
